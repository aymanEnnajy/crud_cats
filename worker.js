export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/cats" && method === "GET") {
      try {
        const { results } = await env.proj_full_db.prepare("SELECT * FROM cats").all();
    ...
    if (url.pathname === "/api/cats" && method === "POST") {
      try {
        ...
        const { lastInsertRowid } = await env.proj_full_db
          .prepare(
        ...
        const { results } = await env.proj_full_db.prepare("SELECT * FROM cats WHERE id=?").bind(lastInsertRowid).all();
    ...
    if (url.pathname.startsWith("/api/cats/") && method === "PUT") {
      try {
        ...
        const { results } = await env.proj_full_db.prepare("SELECT * FROM cats WHERE id=?").bind(id).all();
    ...
    if (url.pathname.startsWith("/api/cats/") && method === "DELETE") {
      try {
        const id = url.pathname.split("/").pop();
        await env.proj_full_db.prepare("DELETE FROM cats WHERE id=?").bind(id).run();
    ...
    if (url.pathname === "/api/auth/register" && method === "POST") {
      try {
        ...
        const { results: existingUsers } = await env.proj_full_db
    ...
    if (url.pathname === "/api/auth/login" && method === "POST") {
      try {
        ...
        const { results: users } = await env.proj_full_db
